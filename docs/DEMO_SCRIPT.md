...
Start with `pnpm demo:preflight`: it checks the live deployment the way a judge meets it (build, faucet on, faucet balance with `--faucet <address>`, an open pool with match left through judging, the database, the Pyth price, the pages and their disclaimer, the security headers, the marketing site's link, and whether the repository is public) and exits 1 if anything a judge needs is missing. `pnpm demo:preflight --live` then runs a real faucet → deposit → claim → withdraw → close round trip on devnet with a throwaway wallet, records each transaction in the activity feed, and prints how long each step took on the real network. The manual items below are what it cannot check.
