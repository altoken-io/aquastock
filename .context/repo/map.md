...
- `apps/dapp`: the actual product. **Current build state: a placeholder only** — `src/app/[locale]/page.tsx` renders a brand-aligned "coming soon" home page inside `components/dapp-shell.tsx` (brand mark + wordmark, theme/language switchers, a link back to `apps/web`; no real navigation yet). No wallet-connect, no project browsing, no funding flow, no My Impact page exist yet — those are Day 1-3 work per the team's build plan. `@solana/web3.js` is an installed dependency; `@solana/wallet-adapter-*` is not yet added. Brand mark/favicon/OG image are rendered as code (`@aquastock/ui/brand/mark` + `next/og`), shared with `apps/web` — see `docs/ASSETS.md`.
