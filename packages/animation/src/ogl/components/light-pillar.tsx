...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  // Check WebGL support. This syncs a browser capability (unavailable during
  // SSR/render) into state, so the setState can't be moved out of the effect.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- renderer creation can only be attempted inside this effect; falling back to the unsupported UI on failure is the sync itself.
      setWebGLSupported(false);
      return;
    }
    ...
    // Only re-run when WebGL support or quality changes; the color/geometry
    // props below seed the initial uniforms and are kept in sync afterward by
    // their own dedicated effects, so intentionally omitting them here avoids
    // tearing down and recreating the whole renderer on every prop change.
